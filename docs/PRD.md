...
- 命中路径：`user_refund_cnt_24h >= 3`（`RC_AS_002` 30 分）+ `user_refund_amount_24h >= 2000`（`RC_AMT_003` 30 分）+ `address_refund_cnt_7d >= 3`（`RC_AS_004` 15 分）→ 规则分 75，落在 60~79 → `Review` 建案。
  三者的分值刻意配成"缺一不可"：只有金额（30）或只有地址聚集（15）都够不到人审线，必须"连续退款 + 大额 + 团伙地址"同时成立才建案——单笔退款证据不足、连续退款叠加才够格，这正是本场景要演示的判定分寸。地址聚集用 `address_refund_cnt_7d`（同地址退款笔数）而不是 `address_refund_rate_7d`：后者是比率，单个地址在 7 天内只下过一两单时会被放大成 100%，作为规则条件容易误伤写字楼前台、合租房等合法共用地址；比率仍作为模型特征参与打分。
...
> **阶段边界说明（2026-09-24 更新）**：P0 阶段已**提前完成** P3 行中的前三项 ——
> 全量数据集（`scripts/gen_dataset.py`）、作弊场景脚本（`app/simulator/scenarios.py` 三个场景）、
...
> * **P1-b（已完成）**：审核工作台前端三栏联调（§11.3）、`DESIGN.md` 中工作台部分的落地。
>   后端的详情接口把中栏需要的六块数据（画像 / 单据 / 特征分组 / 图谱 / 证据链 / 时间线）
>   一次返回，前端不拼多接口。
>
>   验证方式分两层，缺一不可：
>   * **接口层**：`verify_p1.ps1` 36 项断言（建案 → 证据 → 接手 → 处置 → 联动 → 归档 → 审计链）；
>   * **界面层**：无头浏览器（CDP）真实登录三种角色，走完"搜索案件 → 选中 → 接手 →
>     勾选动作 → 填备注 → 二次确认 → 读联动结果"，并回查数据库确认联动真的落库。
>     之所以两层都要有：接口层证明"能力存在"，界面层证明"前端把能力用对了"——
>     门禁文案是否与后端一致、按钮 disabled 条件是否正确、联动结果三档是否渲染，
>     这三类缺陷接口断言**看不见**。
