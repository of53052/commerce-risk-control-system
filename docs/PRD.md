...
| MySQL | `127.0.0.1:3306`，库 `risk_control` | 账号 `root` / `123456`（FlyEnv 托管） |
| Redis | `127.0.0.1:6379` | FlyEnv 托管 |
...
| # | 风险 / 待确认 | 影响 | 应对 |
| --- | --- | --- | --- |
| 1 | FlyEnv 托管的 MySQL / Redis 需保持运行 | 后端不可用 | `dev.ps1` 启动前做依赖健康检查并给出明确提示 |
